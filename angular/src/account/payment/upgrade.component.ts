...
export class UpgradeEditionComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.appSession.tenant.edition.isFree) {
            this._tenantRegistrationAppService.getEdition(this.upgradeEditionId).subscribe((upgradeEdition) => {
                if (this._editionHelperService.isEditionFree(upgradeEdition)) {
                    this._paymentAppService.switchBetweenFreeEditions(upgradeEdition.id).subscribe(() => {
        ...
        this._paymentAppService.hasAnyPayment().subscribe((hasPayment) => {
            if (!hasPayment) {
                this.hideMainSpinner();
                this.redirectToBuy();
                return;
            ...
            this._paymentAppService.getPaymentInfo(this.upgradeEditionId).subscribe((result: PaymentInfoDto) => {
                ...
                if (this.additionalPrice < AppConsts.MinimumUpgradePaymentAmount) {
                    this._paymentAppService
                        .upgradeSubscriptionCostsLessThenMinAmount(this.upgradeEditionId)
                        .subscribe(() => {
    ...
    checkout(gatewayType) {
        ...
        input.successUrl =
            ...
            this._paymentHelperService.getEditionPaymentType(this.editionPaymentType) +
            'Succeed';
